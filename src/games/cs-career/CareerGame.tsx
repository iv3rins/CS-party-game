...
function TournamentFlow({ state, setState }:{state:CareerState;setState:(next:CareerState)=>void}){
  ...
  return <article className="tournament-flow-panel">
    ...
    <div className="live-tournament-list" aria-live="polite">
      {progress.results.map((item,index)=><div key={item.id} className={`live-tournament-row ${index===progress.results.length-1&&revealing?'is-revealing':''}`}><InlineTournamentBadge tier={item.tier} honorClass={item.honorClass}/><div><b>{item.name}</b><small>{item.organizer} · {item.invitationReason}</small></div><strong>{item.placement}</strong><span>{item.wins}/{item.matches}</span><em>{item.rating}</em></div>)}
...
function OutcomeDecision({state,decision,onCommit}:{state:CareerState;decision:Decision;onCommit:(optionId:string,preview:OutcomePreview)=>void}){
  ...
  if(stage==='choices')return <div className="decision-options probability-options">{decision.options.map(item=><button key={item.id} onClick={()=>choose(item.id)}><span>{item.label}</span><small>{item.detail}</small>{item.outcomes?.length&&<span className="option-probabilities">{item.outcomes.map(outcome=>`${outcome.probability}% ${outcome.label}`).join(' / ')}</span>}<em>{item.outcomes?.some(outcome=>outcome.delayed)?'含隐性长期风险':''}</em><ChevronRight/></button>)}</div>;
...
function ActiveCareer({ state, setState, onRestart }: { state: CareerState; setState: (next: CareerState) => void; onRestart: () => void }) {
  ...
  const [mvpCelebration, setMvpCelebration] = useState<{ tournament: CareerState['history'][number]['tournaments'][number]; honor: CareerState['honors'][number] } | null>(null);
  const shownHonorIds = useRef(new Set<string>());
  ...
  useEffect(() => {
    if (state.phase === 'report' && report) {
      ...
      if (lastHonor && (lastHonor.kind === 'MVP' || lastHonor.kind === 'EVP')) {
        ...
        if (tournament && !shownHonorIds.current.has(lastHonor.id)) {
          shownHonorIds.current.add(lastHonor.id);
          setMvpCelebration({ tournament, honor: lastHonor });
        }
      }
    }
  }, [state.phase, report, state.honors]);
  ...
  return <main className="career-dossier">
    ...
    <div className="dossier-grid compact-career-grid">
      ...
      <aside className="career-status compact-status"><section className="player-summary"><RoleIcon role={state.role}/><div><p className="eyebrow">PLAYER DOSSIER</p><h3>{state.name}</h3><span>{state.age}岁 · {role.name} · {state.employmentStatus==='streamer'?'无所属战队':state.team}</span></div></section><section className="career-score"><div><span>目前生涯评分</span><b>{careerScore.score}<small>/100</small></b></div><strong>{careerScore.tier}</strong><p>下一目标：{careerScore.next}</p></section><dl className="career-glance"><div><dt>赛场</dt><dd>{state.tier}</dd></div><div><dt>世界排名</dt><dd>{state.employmentStatus==='streamer'||!state.vrsActive?'未入榜':`#${state.globalRank}`}</dd></div><div><dt>当前Rating</dt><dd>{state.history.at(-1)?.rating??state.stats.rating}</dd></div><div><dt>健康</dt><dd>{state.health}</dd></div><div><dt>合同</dt><dd>{state.employmentStatus==='signed'?`${state.contractHalfSeasonsRemaining/2} 年`:state.employmentStatus==='streamer'?'主播待业':'自由人'}</dd></div><div><dt>可用资产</dt><dd className={state.assets<0?'asset-negative':''}>{Math.round(state.assets)} 万</dd></div><div><dt>潜力状态</dt><dd>{state.highPotential?'年轻高潜':'常规发展'}</dd></div></dl>{state.marketHeat&&<div className="agent-signal"><b>市场热度上升</b><span>{state.marketHeat}</span></div>}<details className="status-details talent-details"><summary>初始天资</summary><div className="talent-list">{state.talents.map(talent=><div key={talent.id}><b>{talent.name}</b><span>{talent.description}</span></div>)}</div></details><Track name={trackLabels.ability} value={state.ability} tone="ability"/><details className="status-details"><summary>{state.employmentStatus==='streamer'?'当前无职业阵容':'阵容与状态'}</summary>{state.roster.length>0?<><div className="roster-list">{state.roster.map(player => <div className={player.isPlayer ? 'active-player' : ''} key={`${player.nick}-${player.role}`}><b>{player.nick}</b><span>{player.role}{player.isPlayer ? ' / 你' : ` · 能力 ${player.ability} · Rating ${player.seasonPerformances.at(-1)?.rating ?? '待赛'}`}{!player.isPlayer&&player.top20History.length ? ` · TOP #${player.top20History.at(-1)?.rank}` : ''}</span></div>)}</div><p>战队状态 {state.teamForm} · 阵容稳定 {state.rosterStability} · 国际适应 {state.internationalAdaptation}</p></>:<p>当前为主播待业状态，没有正式职业阵容。</p>}</details><details className="status-details"><summary>收入与荣誉</summary><p>累计收入 {Math.round(state.stats.earnings)} 万 · 可用资产 {Math.round(state.assets)} 万 · 月薪 {state.salary} 万</p><div className="honor-cabinet">{state.honors.length ? state.honors.slice(-5).reverse().map(honor => <span key={honor.id}><b>{honor.kind}</b>{honor.tournamentName}</span>) : <small>荣誉柜还是空的</small>}</div></details><details className="status-details"><summary>最近事件</summary><div className="field-log">{state.log.slice(0,5).map((item,index) => <span key={`${item}-${index}`}>{item}</span>)}</div></details></aside>
    </div>
