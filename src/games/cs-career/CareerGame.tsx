...
const changeLabels:Partial<Record<keyof StatChange,string>>={ability:'能力',connections:'关系',integrity:'清白',fame:'名气',health:'健康',earnings:'收入',assets:'可用资产',teamForm:'战队状态',rosterStability:'阵容稳定',positionFamiliarity:'位置熟练',internationalAdaptation:'国际适应',highPressureChokingRisk:'高压风险'};
...
function OutcomeDecision({state,decision,onCommit}:{state:CareerState;decision:Decision;onCommit:(optionId:string,preview:OutcomePreview)=>void}){
  ...
  const choose=(optionId:string)=>{
    ...
    if(!next)return;
    setSelected(optionId);setPreview(next);setStage(mobile?'revealed':'rolling');setTrackX(0);
    if(!mobile)timer.current=window.setTimeout(()=>setStage('revealed'),2500);
  };
  const reveal=()=>{
  ...
  return <div className={`outcome-slot ${stage}`} ref={resultRef} tabIndex={-1}>
