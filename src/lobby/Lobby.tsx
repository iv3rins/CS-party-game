  ...
  return <main className="lobby-shell">
    <header className="arena-lobby-bar">
      ...
      <div className="arena-bar-actions">
        ...
        <span className="connection-state" title="当前版本的匹配、房间与天梯仅保存在本机"><i /><Wifi /> 本地体验</span>
    ...
    <div className="arena-lobby-grid">
      ...
      <aside className="lobby-action-panel">
        ...
        <section className="selected-game-action"><small>{multiplayer ? 'LOCAL PVP PREVIEW' : 'CAREER MODE'}</small><h2>{selectedGame.name}</h2><p>{multiplayer ? '前端体验版：模拟匹配和房间仅在当前浏览器有效，对手由本地 AI 驱动。' : '从 16 岁开始，继续你的职业选手档案。'}</p>
          {multiplayer ? <><span className="local-preview-note">本地模拟 · 不跨设备同步</span><button className="lobby-main-command" onClick={() => void startQueue()}><Users />模拟匹配</button><div className="lobby-room-commands"><button onClick={() => setModal('create')}>本地建房</button><button onClick={() => setModal('join')}>本地房间码</button></div></> : <button className="lobby-main-command" onClick={() => void enterGame()}><UserRound />开始游玩</button>}
        </section>
    ...
    {queue && <div className="modal-backdrop"><section className="queue-modal" role="dialog" aria-modal="true">{queue.status === 'searching' ? <><span className="queue-spinner" /><small>LOCAL MATCHMAKING PREVIEW</small><h2>正在模拟匹配</h2><p>预计等待约 {queue.estimatedWaitSeconds} 秒；当前不会连接其他设备。</p><div className="queue-bar"><i /></div><button onClick={async () => setQueue(await platform.cancelQueue(queue.queueId))}>取消匹配</button></> : queue.status === 'matched' ? <><h2>对手已找到</h2><button className="lobby-main-command" onClick={() => void enterGame()}>进入对局</button></> : <><h2>匹配已取消</h2><button onClick={() => setQueue(null)}>返回大厅</button></>}</section></div>}
