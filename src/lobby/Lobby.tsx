...
function BilibiliAttribution() {
  return <a className="bilibili-attribution" href="https://space.bilibili.com/152310954" target="_blank" rel="noreferrer" aria-label="作者 IVERINS 的 Bilibili 主页"><svg viewBox="0 0 24 24" role="img" aria-hidden="true"><path d="M6 7.5h12a3 3 0 0 1 3 3v5a3 3 0 0 1-3 3H6a3 3 0 0 1-3-3v-5a3 3 0 0 1 3-3Z"/><path d="m8 4 2 2m6-2-2 2M8.5 12v2m7-2v2"/></svg><span>作者 IVERINS · Bilibili</span></a>;
}
...
function Lobby() {
  ...
  const [roomType, setRoomType] = useState<'Private' | 'Matchmade' | 'PVE'>('Private');
  ...
  const createRoom = async () => { try { const next = await platform.createRoom({ gameId: selectedGame.gameId, seasonId: selectedGame.seasonId, config: { name: roomName, type: roomType, visibility, roundSeconds: 180, allowSpectators } }); setRoom(next); setModal('room'); } catch (caught) { setError(caught instanceof Error ? caught.message : '创建房间失败'); } };
  ...
  return <main className="lobby-shell">
    ...
    {queue && <div className="modal-backdrop"><section className="queue-modal" role="dialog" aria-modal="true">{queue.status === 'searching' ? <><span className="queue-spinner" /><small>ONLINE MATCHMAKING</small><h2>正在寻找真人对手</h2><p>匹配只会在另一位在线玩家加入后完成，不会生成 AI 对手。</p><div className="queue-bar"><i /></div><button onClick={async () => { try { setQueue(await platform.cancelQueue(queue.queueId)); } catch (caught) { setError(caught instanceof Error ? caught.message : '取消匹配失败'); } }}>取消匹配</button></> : queue.status === 'matched' ? <><small>READY CHECK</small><h2>真人对手已找到</h2><p>请在 15 秒内确认接受，双方确认后进入对局。</p><button className="match-accept-button" onClick={() => void acceptQueue()}>接受匹配</button></> : queue.status === 'accepted' ? <><span className="queue-spinner" /><small>READY CHECK</small><h2>已接受匹配</h2><p>正在等待对手确认。</p></> : queue.status === 'playing' ? <><span className="queue-spinner" /><h2>正在进入对局</h2></> : <><h2>匹配已取消</h2><button onClick={() => setQueue(null)}>返回大厅</button></>}</section></div>}
    ...
    {modal === 'create' && <div className="modal-backdrop"><section className="room-modal" role="dialog" aria-modal="true"><button className="modal-close" aria-label="关闭" onClick={() => setModal(null)}><X /></button><small>PRIVATE ONLINE ROOM</small><h2>创建私人房间</h2><p>创建后会生成六位房间码。私人房供好友加入，PVE 房由 AI 对手陪练。</p><label>房间类型<select value={roomType} onChange={event => setRoomType(event.target.value as 'Private' | 'Matchmade' | 'PVE')}><option value="Private">Private · 私人房</option><option value="Matchmade">Matchmade · 匹配房</option><option value="PVE">PVE · AI 陪练</option></select></label><label>房间名称<input value={roomName} onChange={event => setRoomName(event.target.value)} placeholder="今晚开黑" maxLength={30} /></label><button className="lobby-main-command" onClick={() => void createRoom()}>创建房间</button></section></div>}
