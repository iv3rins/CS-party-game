...
export class QueueService {
  ...
  async current(principalId: string) { return this.repository.getQueue(principalId); }
  ...
  async accept(matchId:string,principalId:string){const check=this.readyChecks.get(matchId);if(!check||this.now()>check.deadline)throw new ServiceError('READY_EXPIRED','匹配确认已过期');if(!check.entries.some(entry=>entry.principal.id===principalId))throw new ServiceError('NOT_PARTICIPANT','不是该匹配参与者');check.accepted.add(principalId);if(check.accepted.size<2)return null;this.readyChecks.delete(matchId);const [first,second]=check.entries;const swap=randomInt(2)===1;const participants: MatchRecord['participants']=swap?[{principal:first.principal,side:'t'},{principal:second.principal,side:'ct'}]:[{principal:first.principal,side:'ct'},{principal:second.principal,side:'t'}];const match:MatchRecord={id:matchId,mode:first.mode,status:'playing',seed:randomUUID(),rulesVersion:'cs-push-v1',participants};await this.repository.saveMatch(match);for(const entry of check.entries){await this.repository.releaseActivity(entry.principal.id,matchId);await this.repository.claimActivity({principalId:entry.principal.id,kind:'match',referenceId:matchId});}return match;}
