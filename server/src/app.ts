...
export const buildApp=async(options:AppOptions)=>{
  ...
  app.post('/api/queues',async(request,reply)=>{const user=await principal(request,reply);if(!user)return;try{const data=body(request);validateGamePartition(String(data.gameId),String(data.seasonId));if(data.mode!=='casual'&&data.mode!=='ranked')throw new ServiceError('INVALID_MODE','模式无效');return await queues.join(user,data.mode);}catch(error){return fail(reply,error);}});
  ...
  app.post('/api/rooms/:roomId/start',async(request,reply)=>{const user=await principal(request,reply);if(!user)return;try{return await rooms.start((request.params as {roomId:string}).roomId,user.id);}catch(error){return fail(reply,error);}});
