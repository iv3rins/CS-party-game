...
export class MatchRuntime {
  ...
  async tick(){if(this.finished)return;for(const side of ['player','ai'] as const){const disconnected=this.disconnectedAt[side];if(disconnected!==undefined&&this.clock.now()-disconnected>=30_000){await this.finish(side==='player'?'t':'ct','disconnect');return;}if(!this.connected[side])this.aiMove(side);}
    this.state=tickGame(this.state,.1,()=>this.rng.next());this.sequence+=1;this.emit({type:'snapshot',snapshot:this.snapshot()});if(this.state.status!=='playing'){const outcome=this.state.status==='draw'?'draw':this.state.status==='player-win'?'ct':'t';await this.finish(outcome,this.state.elapsed>=240?'time':'base');}
  }
